...
BOOST_AUTO_TEST_CASE(TestEngine)
{
  ...
  if(testDefend.validate(testEngine.getTurn())){
    std::shared_ptr<Defend> ptr_defend (new Defend(testDefend));
  ...
  //   std::shared_ptr<Attack> ptr_attack (new Attack(testAttack));
...
//    std::shared_ptr<UseSkill> ptr_testUseSkill (new UseSkill(testUseSkill));
...
//  BOOST_CHECK_EQUAL(testEngine.getTurn().getTeams()[0]->getListCharacter()[2]->getStatus(),Used);
  ...
  if(testUseItem.validate(testEngine.getTurn())){
    std::shared_ptr<UseObject> ptr_testUseItem (new UseObject(testUseItem));
  ...
  //   std::shared_ptr<Attack> ptr_attack2 (new Attack(testAttack2));
  ...
  // BOOST_CHECK_EQUAL(testEngine.getTurn().getTeams()[0]->getListCharacter()[5]->getStatus(),Available);
    ...
    //     std::shared_ptr<Move> ptr_movetest (new Move(movetest));
    ...
    if(i!=4){
      ...
      if(testDefend2.validate(testEngine.getTurn())){
        std::shared_ptr<Defend> ptr_defend2 (new Defend(testDefend2));
    ...
    if(endturnTest.validate(testEngine.getTurn())){
      std::shared_ptr<EndTurn> ptr_endTestTurn (new EndTurn(endturnTest));
